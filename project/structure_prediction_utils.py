...
def load_preprocessed_data(data_folder, filename, batch_size):
    file_path = data_folder+filename
    if not os.path.exists(file_path):
        raise(ValueError(f"no data found in: {file_path}"))

    raw_dataset = tf.data.TFRecordDataset(file_path)
    decoded_dataset = raw_dataset.map(decode_preprocessed_fn).shuffle(buffer_size=256).batch(batch_size).prefetch(tf.data.AUTOTUNE)
    #debug info
